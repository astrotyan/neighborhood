# Two bugs in branca fixed by Ting Yan.
# Replace /usr/local/lib/python3.7/site-packages/branca/utilities.py with this file.

# 1. _cnames.json should be replaced by scheme_info.json
# 2. linear_gradient(schemes.get(core_color_code), n) should be
#    linear_gradient(schemes.get(base_code +
#                    '_' + str(max(matching_quals)).zfill(2)), n)
    ...
    if not explicit_scheme:
        ...
        if scheme_info[base_code] == 'Qualitative':
            ...
            if not color_reverse:
                color_scheme = linear_gradient(schemes.get(base_code +
                                            '_' + str(max(matching_quals)).zfill(2)), n)
            else:
